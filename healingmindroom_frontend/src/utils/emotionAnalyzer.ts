...
const DEFAULT_COMFORT_TEMPLATES: Record<string, Record<string, string>> = {
  ...
  '40s': {
    '스트레스': '가정과 직장의 모든 책임을 짊어지느라 지친 어깨를 토닥여 드리고 싶습니다. 온전히 자신만을 돌보는 치유공간으로 떠나보시는 건 어떨까요.',
  ...
  '60s': {
    ...
    '미래 고민': '걸어오신 길은 충분히 훌륭합니다. 앞으로 마주할 내일도 편안하고 안전할 수 있도록 자연 속 넉넉한 치유공간이 당신을 감싸 안아줄 것입니다.',
    ...
    try {
      ...
      if (response.ok) {
        ...
        return {
          ...
          comfortMessage: nlpResult.comfortMessage || '힘든 마음을 들어주고 함께 할 치유공간을 소개합니다.'
        };
      }
    } catch (e) {
